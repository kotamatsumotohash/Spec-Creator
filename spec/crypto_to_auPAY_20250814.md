...
- 残高不足：「暗号資産の残高が不足しています」＋ウォレット画面に戻るボタン
- 月間制限超過：「今月の利用上限に達しています（上限：50,000円）」＋OK/履歴確認ボタン
- ネットワークエラー：「通信エラーが発生しました」＋再試行/キャンセルボタン
- クオート期限切れ：「価格の有効期限が切れました」＋新しい価格で再開ボタン
- システムメンテナンス：「システムメンテナンス中です。しばらくお待ちください」＋OK/メンテナンス詳細ボタン
...
| Must | エラーハンドリング | 下記エラーパターンに対応したダイアログ表示・適切なアクション提供 |
...
| --- | --- | --- | --- | --- |
| 残高不足 | BTC/USDC残高 < チャージ必要額 | 「暗号資産の残高が不足しています」 | ウォレット画面に戻る | 残高確認後、金額選択画面に戻る |
| 月間制限超過 | 月間累計 + チャージ額 > 50,000円 | 「今月の利用上限に達しています（上限：50,000円）」 | OK/履歴確認 | 利用履歴画面への遷移可能 |
| ネットワークエラー | API通信失敗・タイムアウト | 「通信エラーが発生しました」 | 再試行/キャンセル | 3回まで自動リトライ、その後手動 |
| クオート期限切れ | 30秒のタイムアウト | 「価格の有効期限が切れました」 | 新しい価格で再開 | 最新価格を取得して金額選択画面に戻る |
| システムメンテナンス | API側メンテナンスモード | 「システムメンテナンス中です。しばらくお待ちください」 | OK/メンテナンス詳細 | メンテナンス情報表示・時間外アクセス制御 |
| KYC未完了 | auPAY側KYCステータス未完了 | 「本人確認が必要です」 | KYC手続きに進む/後で | auPAY設定画面への遷移 |
| 不正検知 | 異常なアクセスパターン検出 | 「セキュリティ確認のため一時的に利用を停止しています」 | カスタマーサポートに連絡 | アカウントロック・サポートへエスカレーション |
| 処理中エラー | 決済処理中の例外発生 | 「処理中にエラーが発生しました」 | 再試行/キャンセル | トランザクションロールバック |
...
| エラー通知・アラート設計 | 各エラーパターンでの運用チーム通知・エスカレーション基準 |
